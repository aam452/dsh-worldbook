...
- 对方插件：`@dsh-external/dsh-agent-rp`（私有包，当前安装版本 `0.0.0-rc.193`，MIT），对方经常更新，**不能改对方源码**，所有兼容靠 DSH 宿主机制 + 只读适配。
- 我方插件：`dsh-worldbook`（本仓库）。
- **通用兼容已实现**：世界书接管协议（`src/integration/`，权威见 `DEVELOPMENT.md` 第七章）已把 `worldbook.engine` / `worldbook.operations` 的**提供**、`worldbook.context` / `worldbook.source` 的**消费侧**做好。**本模块只做 agent-rp 专属适配**（提供 source/context/character-books、拦截、命令影子、迁移），通用侧不再重复实现。
...
| `worldbook.character-books` 提供：当前角色卡世界书管理投影 | 本模块 | ✅ `character.ts` |
...
通用侧的消费方管线（`resolveSessionInjection` → `renderWorldbookInjection`）对我们已经就位：本模块只要把「对方会话里有哪些书、当前角色是谁」通过标准服务键交出去，注入、角色过滤、同名去重全部由通用侧完成。角色卡管理投影通过独立的 `worldbook.character-books` 服务提供。

## 1. agent-rp的插件是怎么工作的（要点速查）
...
**注入侧已由通用兼容接管**：适配器内部读取 agent-rp 事件后，通过 `worldbook.source.readBooks(sessionId)` 返回本会话书籍；通用注入引擎只注入本库书（全局 + 绑定书）和适配器返回的书。**不需要为 agent-rp 单独写注入管线。**
...
1. **旧会话历史书**：开兼容之前事件流里已有的书，对方仍注入。→ 迁移后开新会话才干净。
2. **脚本新建并绑定的世界书（硬边界）**：Tavern Helper 脚本可**新建**书并绑定，走 `rp-tavern-variables` 命令 + 状态（tavern-helper.ts worldbooks/deletedWorldbookNames/worldbookBindings），只能读不能拦。**脚本新造的书对方仍注入。**
...
src/compat/agent-rp/   ← 本模块（已实现），只做 agent-rp 专属适配
  ...
  character.ts         ← 提供 worldbook.context 与 worldbook.character-books：桥接角色上下文和角色卡世界书投影
...
- 对方是 `0.0.0-rc.193` 私有包、频繁更新：**所有解析对方事件/状态的逻辑必须隔离在本目录适配器里**，并用冒烟测试锁事件格式（参照 scripts/worldbook-*.mjs 的写法）。
