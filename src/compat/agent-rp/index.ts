...
let disposeAgentRp: (() => void) | null = null

export function syncAgentRpCompat(ctx: Context): void {
  ...
  if (should && disposeAgentRp === null) {
    ...
    if (repairedNames.length > 0) ctx.logger.info(`[dsh-worldbook] agent-rp 映射修复: 已拆分 ${repairedNames.join('、')}`)
    ...
    if (stopWatch) disposers.push(stopWatch)
    disposeAgentRp = () => {
