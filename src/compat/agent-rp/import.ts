...
export function mappedBookId(sourceKey: string): string | undefined {
  return loadMap()[sourceKey]
}

/** sourceKey → 本库当前书名（改名后跟随）；未迁移返回 undefined。 */
export function sourceBookName(sourceKey: string): string | undefined {
...
export function mapSourceBook(sourceKey: string, bookId: string): void {
  const map = loadMap()
...
const sessionBooks = new Map<string, Set<string>>()

export function rememberSessionBook(sessionId: string, name: string): void {
...
export function sessionActiveBooks(sessionId: string): string[] {
...
export function ensureSessionCardBook(sessionId: string, events: readonly unknown[]): string | undefined {
  let card: ReturnType<typeof activeCharacterBook>
  try {
...
// ── 对方宿主世界书库（shelf）同步：对方「导入世界书」上传到库里的书，同步迁入本库。 ──
...
export function readLibraryAsset(importId: string): LibraryAsset | null {
  if (!LIBRARY_ID.test(importId)) return null
...
export function migrateLibraryShelf(): string[] {
  ...
  for (const filename of files) {
    ...
    if (isMigrated(sourceKey)) continue
...
export function watchLibraryShelf(onChange: (names: string[]) => void): (() => void) | null {
  if (!existsSync(LIBRARY_ROOT)) return null
  try {
    const watcher = watch(LIBRARY_ROOT, () => {
      try {
        const names = migrateLibraryShelf()
        if (names.length > 0) onChange(names)
      } catch {
        // 监听回调失败忽略
      }
    })
