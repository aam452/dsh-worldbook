...
import type { SessionBook } from './events.js'
...
export function applyAgentRpDebugLogger(ctx: Context): (() => void) | null {
  ...
  return ctx.on('agent/pre-step', async ({ agent }, next) => {
    try {
      if (!setting.agentRpDebug()) return next()
      if (!agentRpActiveInSession(agent)) return next()
      const now = Date.now()
      ...
      try {
        for (const book of source.readBooks(agent.session.events) as SessionBook[]) {
      ...
      ctx.logger.info(
        `[dsh-worldbook][agent-rp] 会话=${agent.id} 角色=${character ?? '无'} ` +
