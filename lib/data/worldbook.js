...
export function toEntryView(row) {
    ...
    const rawMap = raw ?? {};
    const keys = parseJson(row.keys, []);
    const secondary = parseJson(row.secondary_keys, []);
    ...
    const view = {
        ...
        selectiveLogic: row.selective_logic ?? 0,
...
export function replaceEntries(bookId, items) {
    ...
    const insert = db.prepare(`INSERT INTO worldbook_entries (id, worldbook_id, keys, secondary_keys, comment, content, constant, vectorized, selective, selective_logic, insertion_order, position, enabled, priority, case_sensitive, match_whole_words, scan_depth, exclude_recursion, prevent_recursion, use_probability, probability, depth, sticky, cooldown, delay, display_index, raw, created_at, updated_at, created_by, updated_by, is_deleted, deleted_at)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,0,NULL)`);
    items.forEach((item, index) => {
        const v = normalizeEntry(item);
        if (item.displayIndex === undefined)
            v.displayIndex = index;
        insert.run(uuid(), bookId, toJson(v.keys), toJson(v.secondary_keys), v.comment, v.content, v.constant ? 1 : 0, v.vectorized ? 1 : 0, v.selective ? 1 : 0, v.selectiveLogic, v.insertionOrder, v.position, v.enabled ? 1 : 0, v.priority, v.caseSensitive === null ? null : v.caseSensitive ? 1 : 0, v.matchWholeWords === null ? null : v.matchWholeWords ? 1 : 0, v.scanDepth, v.excludeRecursion ? 1 : 0, v.preventRecursion ? 1 : 0, v.useProbability ? 1 : 0, v.probability, v.depth, v.sticky, v.cooldown, v.delay, v.displayIndex, item.raw !== undefined && item.raw !== null ? toJson(item.raw) : null, t, t, 'user', 'user');
    });
}
function strArray(v) {
...
export function normalizeEntry(src) {
    ...
    return {
        ...
        insertionOrder: num(src.insertionOrder ?? src.order ?? raw?.insertion_order, 100) ?? 100,
        position,
...
export function addEntry(bookId, patch) {
    ...
    if (src.displayIndex === undefined) {
        const max = getDb().prepare('SELECT COALESCE(MAX(display_index), -1) AS m FROM worldbook_entries WHERE worldbook_id=? AND is_deleted=0').get(bookId);
    ...
    const t = now();
    const id = uuid();
    db.prepare(`INSERT INTO worldbook_entries (id, worldbook_id, keys, secondary_keys, comment, content, constant, vectorized, selective, selective_logic, insertion_order, position, enabled, priority, case_sensitive, match_whole_words, scan_depth, exclude_recursion, prevent_recursion, use_probability, probability, depth, sticky, cooldown, delay, display_index, raw, created_at, updated_at, created_by, updated_by, is_deleted, deleted_at)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,0,NULL)`).run(id, bookId, toJson(v.keys), toJson(v.secondary_keys), v.comment, v.content, v.constant ? 1 : 0, v.vectorized ? 1 : 0, v.selective ? 1 : 0, v.selectiveLogic, v.insertionOrder, v.position, v.enabled ? 1 : 0, v.priority, v.caseSensitive === null ? null : v.caseSensitive ? 1 : 0, v.matchWholeWords === null ? null : v.matchWholeWords ? 1 : 0, v.scanDepth, v.excludeRecursion ? 1 : 0, v.preventRecursion ? 1 : 0, v.useProbability ? 1 : 0, v.probability, v.depth, v.sticky, v.cooldown, v.delay, v.displayIndex, toJson(src.raw), t, t, 'user', 'user');
...
export function updateEntry(bookId, entryId, patch) {
    ...
    const db = getDb();
    const t = now();
    db.prepare(`UPDATE worldbook_entries SET keys=?, secondary_keys=?, comment=?, content=?, constant=?, vectorized=?, selective=?, selective_logic=?, insertion_order=?, position=?, enabled=?, priority=?, case_sensitive=?, match_whole_words=?, scan_depth=?, exclude_recursion=?, prevent_recursion=?, use_probability=?, probability=?, depth=?, sticky=?, cooldown=?, delay=?, display_index=?, raw=?, updated_at=? WHERE id=?`).run(toJson(v.keys), toJson(v.secondary_keys), v.comment, v.content, v.constant ? 1 : 0, v.vectorized ? 1 : 0, v.selective ? 1 : 0, v.selectiveLogic, v.insertionOrder, v.position, v.enabled ? 1 : 0, v.priority, v.caseSensitive === null ? null : v.caseSensitive ? 1 : 0, v.matchWholeWords === null ? null : v.matchWholeWords ? 1 : 0, v.scanDepth, v.excludeRecursion ? 1 : 0, v.preventRecursion ? 1 : 0, v.useProbability ? 1 : 0, v.probability, v.depth, v.sticky, v.cooldown, v.delay, v.displayIndex, toJson(src.raw), t, entryId);
...
// 兼容两种形态：spec v2 entries(数组) 与 ST 编辑器内部 entries(对象，键为字符串序数)。
