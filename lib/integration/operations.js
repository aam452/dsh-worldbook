...
import { WORLDBOOK_OPERATIONS_KEY, WorldbookProtocolError, } from './protocol.js';
const deleteGuards = new Set();
const deleteHooks = new Set();
/** 注册一个宿主适配层的删除策略；通用层不识别任何宿主来源格式。 */
export function registerDeleteGuard(guard) {
    deleteGuards.add(guard);
    return () => deleteGuards.delete(guard);
}
export function assertBookDeletable(book) {
    for (const guard of deleteGuards)
        guard(book);
}
export function registerDeleteHook(hook) {
    deleteHooks.add(hook);
    return () => deleteHooks.delete(hook);
}
export function notifyBookDeleted(book) {
...
export function syncOperations(ctx) {
...
export function createOperations() {
    ...
    const entriesInStOrder = (bookId) => [...worldbook.entries(bookId)].sort((a, b) => a.displayIndex - b.displayIndex || a.order - b.order);
    const resolve = (name) => {
        if (typeof name !== 'string' || name.length === 0)
            throw new WorldbookProtocolError('INVALID_ARGUMENT', '世界书名称必须是非空字符串');
        const row = worldbook.findByName(name);
        if (!row)
            throw new WorldbookProtocolError('NOT_FOUND', `世界书「${name}」不存在`);
        return row;
    };
    const resolveEntry = (bookName, entryId) => {
        const row = resolve(bookName);
        const list = entriesInStOrder(row.id);
        const target = list.find(entry => entry.id === entryId);
        if (!target)
            throw new WorldbookProtocolError('NOT_FOUND', `世界书「${bookName}」没有条目「${entryId}」`);
        return { row, entryId: target.id, disable: target.disable === 1 };
    };
    return {
        listBooks: () => worldbook.list().map((row) => ({
            name: row.name,
        ...
        getBook: (name) => {
            ...
            if (parsed.scanDepth !== undefined)
                book.scan_depth = parsed.scanDepth;
            if (parsed.recursiveScanning !== undefined)
                book.recursive_scanning = parsed.recursiveScanning;
            if (parsed.extensions !== undefined)
                book.extensions = parsed.extensions;
            return book;
        },
        createBook: (book) => {
            ...
            if (worldbook.findByName(name))
                throw new WorldbookProtocolError('NAME_CONFLICT', `世界书「${name}」已存在`);
        ...
        updateBook: (name, book) => {
            ...
            if (conflict && conflict.id !== row.id)
                throw new WorldbookProtocolError('NAME_CONFLICT', `世界书「${nextName}」已存在`);
