...
function migrate(instance) {
    const renameCol = (table, from, to, invert) => {
        try {
            ...
            if (hasFrom && !hasTo) {
                ...
                if (invert)
                    instance.exec(`UPDATE ${table} SET "${to}" = 1 - "${to}"`);
            }
        }
        catch {
            // 表不存在等场景忽略
    ...
    try {
        const cols = instance.prepare('PRAGMA table_info(worldbook_entries)').all();
        if (cols.some((c) => c.name === 'priority'))
            instance.exec('ALTER TABLE worldbook_entries DROP COLUMN priority');
    }
    catch {
