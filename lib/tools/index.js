...
export function scopeGuard(args) {
    ...
    if (setting.devAction() === 'edit') {
        ...
        if (!allowedBook && action !== 'list_books') {
            return { ok: false, message: '编辑模式尚未选择允许操作的世界书。' };
        }
        ...
        if (allowedEntries.length > 0 && (action === 'update_entry' || action === 'delete_entry' || action === 'create_entry')) {
            const entryId = typeof args.entryId === 'string' ? args.entryId : '';
            if (action === 'create_entry' || !entryId || !allowedEntries.includes(entryId)) {
        ...
        if (bookId && (action === 'create_entry' || action === 'update_entry' || action === 'delete_entry')) {
...
const worldbookTool = defineTool({
    name: 'worldbook_edit',
    description: '世界书（World Info）编辑工具：管理世界书与条目。开发模式专用，仅当插件设置开启「开发世界书模式」时可用。' +
    ...
    async execute(args) {
        ...
        if (action === 'list_books') {
            ...
            const books = setting.devAction() === 'edit'
                ? (setting.devBookId() ? [worldbook.get(setting.devBookId())].filter((book) => book !== null) : [])
                : worldbook.list();
            return j({ books: books.map(worldbook.toBookView) });
        }
        if (action === 'list_entries') {
            if (!bookId)
                throw new Error('action=list_entries 需要 bookId');
            ...
            const visibleEntries = allowedEntries.length > 0
                ...
                : entries;
            return j({ entries: visibleEntries.map((r) => worldbook.toEntryView(r)) });
        }
        if (action === 'create_book') {
            ...
            if (entry && Object.keys(entry).length > 0)
                entryRow = worldbook.addEntry(row.id, entry);
            return j({ book: worldbook.toBookView(row), entry: entryRow ? worldbook.toEntryView(entryRow) : null });
        }
        if (action === 'create_entry') {
            if (!bookId)
                throw new Error('action=create_entry 需要 bookId');
            const entry = args.entry ?? {};
            const row = worldbook.addEntry(bookId, entry);
            return j({ entry: worldbook.toEntryView(row) });
        }
        if (action === 'update_entry') {
            if (!bookId || typeof args.entryId !== 'string')
                throw new Error('action=update_entry 需要 bookId 和 entryId');
            const patch = pick(args, ['name', 'description']);
            ...
            if (!row)
                return j({ ok: false, message: '条目不存在' });
            return j({ ok: true, entry: worldbook.toEntryView(row) });
        }
        if (action === 'delete_entry') {
            if (!bookId || typeof args.entryId !== 'string')
                throw new Error('action=delete_entry 需要 bookId 和 entryId');
        ...
        if (action === 'export_book') {
            if (!bookId)
                throw new Error('action=export_book 需要 bookId');
            const book = worldbook.get(bookId);
            if (!book)
                return j({ ok: false, message: '世界书不存在' });
            return j({ name: book.name, json: worldbook.toStWorldJson(bookId) });
        }
        throw new Error('未知 action: ' + action);
    },
});
export function syncDevTool(ctx) {
