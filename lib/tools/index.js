...
function guardResult(errorType, message) {
...
const ENTRY_FIELDS = {
    ...
    position: { type: 'integer', enum: [0, 1, 2, 3, 4, 5, 6, 7], description: '注入位置（ST position 枚举：0=before_char 1=after_char 4=@D；list_entries 返回为数字）。' },
...
const worldbookTool = defineTool({
    ...
    async execute(args) {
        ...
        if (action === 'delete_book') {
            if (!bookId)
                throw new Error('action=delete_book 需要 bookId');
