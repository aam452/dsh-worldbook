...
import { WORLDBOOK_OPERATIONS_KEY } from '../../integration/protocol.js';
import { applyConfigurationRequest, encodeWorldInfoConfiguration, parseWorldInfoConfigurationRequest, parseWorldInfoLibraryLaunchRequest, readWorldInfoConfiguration, } from './events.js';
import { ensureSourceBook, readLibraryAsset, rememberSessionBook, sourceBookName } from './import.js';
function resolveBookName(agentRpBookId) {
...
function getOperations(invocation) {
    let operations;
    try {
        operations = invocation.agent.ctx.get(WORLDBOOK_OPERATIONS_KEY);
    }
    catch {
        operations = undefined;
    }
    if (!operations)
        throw new Error('世界书操作接口未启用，请先开启兼容模式中的操作接口');
    return operations;
}
function executeConfiguration(invocation) {
    ...
    if (bookName === undefined)
        throw new Error('目标世界书不存在（未迁移到 dsh-worldbook 库）');
    ...
    if (request.operation === 'reset-book' || request.operation === 'set-book-enabled') {
        ...
        for (const entry of book.entries) {
            if (entry.id)
                operations.toggleEntry(bookName, entry.id, enable);
        }
        return { kind: 'success', text: encodeWorldInfoConfiguration(applyConfigurationRequest(current, request)) };
    }
    const entry = book.entries[request.entryIndex];
    if (entry === undefined)
        throw new Error('目标世界书条目不存在');
    if (!entry.id)
        throw new Error('目标世界书条目缺少稳定标识');
...
function makeCommand(name, handler, description) {
    return {
        name,
        description,
...
export function applyAgentRpCommands(ctx) {
    ...
    const tryRegister = () => {
        ...
        try {
            ...
            if (!commands)
                return;
            registered = true;
            const first = commands.register(makeCommand('rp-world-info', executeConfiguration, 'manage this roleplay Session world info (dsh-worldbook)'));
            const second = commands.register(makeCommand('rp-world-info-import', executeLibraryImport, 'import one Host-owned World Info source (dsh-worldbook)'));
            if (typeof first === 'function')
                disposeRegistrations.push(first);
            if (typeof second === 'function')
                disposeRegistrations.push(second);
        }
        catch (error) {
