...
import { isActiveForSession } from '../data/scope.js';
...
export function registerContextInjection(ctx) {
    let registry;
    try {
    ...
    ctx.on('agent/pre-step', async ({ agent, signal, messages }, next) => {
        ...
        if (!isActiveForSession(registry, sessionId))
            return decision;
        // per-turn 模式：仅当本 step 由用户新输入触发时注入
        if (injectMode() === 'per-turn') {
            const hasUserInput = messages.some((m) => m.source?.kind === 'user');
            if (!hasUserInput)
                return decision;
        }
        ...
        const contextMessages = [...decision.messages];
        ...
        if (contextMessages.length === decision.messages.length)
            return decision;
...
function visibleMessageCursor(agent) {
    let cursor = 0;
    for (const e of agent.session.events) {
        const source = e.data.source;
        if (e.type === 'user/message' && source?.kind === 'user')
            cursor++;
