...
const mk = (comment, patch) => {
  const row = wb.addEntry(book.id, { comment, content: `C:${comment}`, key: [], disable: false, ...patch })
  return row.id
}
const A = mk('A递归触发', { key: ['甲'], content: 'C:A 这里包含乙', excludeRecursion: false })
const B = mk('B被递归命中', { key: ['乙'], excludeRecursion: false })
const C = mk('C排除递归', { key: ['丙'], excludeRecursion: true })
const D = mk('D阻止递归', { key: ['丁'], preventRecursion: true, content: 'C:D 这里包含戊' })
const E = mk('E粘性', { key: ['粘'], sticky: 3 })
const F = mk('F冷却', { key: ['冷'], cooldown: 3 })
...
wb.addEntry(book2.id, { comment: 'A触发', content: 'C:A 包含丙', key: ['甲'], excludeRecursion: false })
wb.addEntry(book2.id, { comment: 'C排除', content: 'C:C 丙条目', key: ['丙'], excludeRecursion: true })
wb.addEntry(book2.id, { comment: 'H对照', content: 'C:H 丙条目', key: ['丙'], excludeRecursion: false })
...
wb.addEntry(book3.id, { comment: 'D阻止', content: 'C:D 这里包含戊', key: ['丁'], preventRecursion: true })
wb.addEntry(book3.id, { comment: 'G戊条目', content: 'C:G 戊条目', key: ['戊'], excludeRecursion: false })
...
wb.addEntry(book6.id, { comment: '延迟3', content: 'C:延迟3', key: ['延迟三'], delay: 3 })
wb.addEntry(book6.id, { comment: '延迟递归', content: 'C:延迟递归', key: ['递归启'], delayUntilRecursion: true, excludeRecursion: false })
wb.addEntry(book6.id, { comment: 'A触发', content: 'C:A 包含递归启', key: ['甲'], excludeRecursion: false })
...
wb.addEntry(book7.id, { comment: 'A1', content: 'C:A1 包含乙', key: ['甲'], excludeRecursion: false })
wb.addEntry(book7.id, { comment: 'A2', content: 'C:A2 包含丙', key: ['乙'], excludeRecursion: false })
wb.addEntry(book7.id, { comment: '延迟1', content: 'C:延迟1', key: ['乙'], delayUntilRecursion: 1, excludeRecursion: false })
wb.addEntry(book7.id, { comment: '延迟2', content: 'C:延迟2', key: ['丙'], delayUntilRecursion: 2, excludeRecursion: false })
