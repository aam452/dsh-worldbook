...
check('导入后 display_index 按序 0..20', rows.every((r, i) => r.display_index === i))
...
check('reorder 后 display_index 重新 0..20', rows.every((r, i) => r.display_index === i))
...
check('addEntry 排在末尾（display_index=21）', wb.toEntryView(added).displayIndex === rows.length)
